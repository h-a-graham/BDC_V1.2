...
list.of.packages <- c("rnrfa", "tidyverse", "minpack.lm", "broom")
...
# 
# below is all Hydrological Area Numbers for GB:
...
# Retrieve Args from command line - i.e from Python's subprocess
req_HA_num <- as.integer(commandArgs(trailingOnly = TRUE)[1]) # use this when running via python

plot.path <- as.character(commandArgs(trailingOnly = TRUE)[2])
